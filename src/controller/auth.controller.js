...
const {
  bcryptServices: { hashPassword, comparePassword },
  jwtServices: { createToken, verifyToken },
} = require("../services");
...
exports.signup = async (req, res, _) => {
  try {
    ...
    if (user) {
      ...
      return res
        .status(201)
        .json({ message: "created successfuly", status: 201 });
    }
  } catch (error) {
    console.log(error);
  }
  ...
  try {
    ...
    if (user) {
      ...
      if (isMatch) {
        ...
        return res
          .status(200)
          .json({ message: "logged in", status: 200, token: token });
      } else {
        return res
          .status(401)
          .json({ message: "invalid credential", status: 401 });
      }
    } else {
